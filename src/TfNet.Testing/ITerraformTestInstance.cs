...
public interface ITerraformTestInstance : IDisposable
{
    ...
    /// <summary>
    /// Runs a Terraform command and returns its output. Fails if the Terraform process does not exit with code 0 within the specified timeout.
    /// </summary>
    /// <param name="command">The command to run.</param>
    /// <param name="timeout">Defines how long to wait for Terraform to exit. Defaults to 1 second if not specified.</param>
    Task<string> RunCommandAsync(string command, TimeSpan? timeout = default);
}
